...
import (
	...
	"github.com/chaos-io/cargo/backend/modules/cargo-server/api/endpoint/middleware"
	grpc2 "github.com/chaos-io/cargo/backend/modules/cargo-server/api/grpc"
	http2 "github.com/chaos-io/cargo/backend/modules/cargo-server/api/http"
	app2 "github.com/chaos-io/cargo/backend/modules/cargo-server/app"
	pb "github.com/chaos-io/cargo/genproto/cargo/v1"
)

const FullServiceName = "cargo.v1.CargoService"

var _ nclient.Config

func NewEndpoints(options map[string]any) endpoint.Endpoints {
	// Business domain.
	var service pb.CargoServiceServer
	{
		service = app2.NewService()
	...
	// Endpoint domain.
	var (
		getCargoEndpoint     = endpoint.MakeGetCargoEndpoint(service)
		createCargoEndpoint  = endpoint.MakeCreateCargoEndpoint(service)
		handingEventEndpoint = endpoint.MakeHandingEventEndpoint(service)
	)

	endpoints := endpoint.Endpoints{
		GetCargoEndpoint:     getCargoEndpoint,
		CreateCargoEndpoint:  createCargoEndpoint,
		HandingEventEndpoint: handingEventEndpoint,
	}
...
func RegisterService(cfg nserver.Config, r *mux.Router, s *grpc.Server) endpoint.Endpoints {
	...
	pb.RegisterCargoServiceServer(s, grpc2.MakeGRPCServer(endpoints, tracer, logger))
