...
func RegisterHttpHandler(router *mux.Router, endpoints endpoint.Endpoints, tracer trace.Tracer, logger log.Logger) {
	...
	router.Methods("GET").Path("/cargo/v1/cargo/{tracking_id}").Handler(
		httptransport.NewServer(
			endpoints.GetCargoEndpoint,
			DecodeHTTPGetCargoRequest,
			EncodeHTTPGenericResponse,
			addTracerOption("GetCargo")...,
		))

	router.Methods("POST").Path("/cargo/v1/cargo").Handler(
		httptransport.NewServer(
			endpoints.CreateCargoEndpoint,
			DecodeHTTPCreateCargoRequest,
			EncodeHTTPGenericResponse,
			addTracerOption("CreateCargo")...,
		))

	router.Methods("POST").Path("/cargo/v1/event").Handler(
		httptransport.NewServer(
			endpoints.HandingEventEndpoint,
			DecodeHTTPHandingEventRequest,
			EncodeHTTPGenericResponse,
...
// DecodeHTTPGetCargoRequest is a transport/http.DecodeRequestFunc that
// decodes a JSON-encoded getcargo request from the HTTP request
// body. Primarily useful in a server.
func DecodeHTTPGetCargoRequest(_ context.Context, r *http.Request) (interface{}, error) {
	var req pb.GetCargoReq
	...
	err = unmarshalPathParam(pathParams, &req.TrackingId, "tracking_id")
	if err != nil && !core.IsNotFoundError(err) {
		return nil, nhttp.WrapError(err, 400, "cannot unmarshal the tracking_id query parameter")
	}

	err = unmarshalQueryParam(queryParams, &req.CustomerId, "customer_id")
	if err != nil && !core.IsNotFoundError(err) {
		return nil, nhttp.WrapError(err, 400, "cannot unmarshal the customer_id query parameter")
	}

	return &req, nil
}

// DecodeHTTPCreateCargoRequest is a transport/http.DecodeRequestFunc that
// decodes a JSON-encoded createcargo request from the HTTP request
// body. Primarily useful in a server.
func DecodeHTTPCreateCargoRequest(_ context.Context, r *http.Request) (interface{}, error) {
	var req pb.CreateCargoReq
	...
	if req.Cargo == nil {
		...
		req.Cargo = &cargo.Cargo{}
	}

	err = unmarshalQueryParam(queryParams, req.Cargo, "cargo")
	if err != nil {
		if core.IsNotFoundError(err) {
			...
			return nil, nhttp.WrapError(err, 400, "cannot unmarshal the cargo query parameter")
		}
	}
...
// DecodeHTTPHandingEventRequest is a transport/http.DecodeRequestFunc that
// decodes a JSON-encoded handingevent request from the HTTP request
// body. Primarily useful in a server.
func DecodeHTTPHandingEventRequest(_ context.Context, r *http.Request) (interface{}, error) {
	var req pb.HandingEventReq
	...
	if req.Event == nil {
		...
		req.Event = &cargo.HandlingEvent{}
	}

	err = unmarshalQueryParam(queryParams, req.Event, "event")
	if err != nil {
		if core.IsNotFoundError(err) {
			if eventInitialized {
				req.Event = nil
			...
			return nil, nhttp.WrapError(err, 400, "cannot unmarshal the event query parameter")
		}
	}
