...
import (
	...
	"github.com/chaos-io/cargo/backend/modules/cargo/api/endpoint/middleware"
	"github.com/chaos-io/cargo/backend/modules/cargo/app"
	http2 "github.com/chaos-io/cargo/backend/modules/cargo/api/http"
	grpc2 "github.com/chaos-io/cargo/backend/modules/cargo/api/grpc"
...
const FullServiceName = "cargo.v1.Cargo"

var _ nclient.Config

func NewEndpoints(options map[string]interface{}) endpoint.Endpoints {
	// Business domain.
	var service pb.CargoServer
	{
	...
	// Endpoint domain.
	var (
		createResourceEndpoint = endpoint.MakeCreateResourceEndpoint(service)
		getResourceEndpoint    = endpoint.MakeGetResourceEndpoint(service)
		listResourcesEndpoint  = endpoint.MakeListResourcesEndpoint(service)
		updateResourceEndpoint = endpoint.MakeUpdateResourceEndpoint(service)
		deleteResourceEndpoint = endpoint.MakeDeleteResourceEndpoint(service)
	)

	endpoints := endpoint.Endpoints{
		CreateResourceEndpoint: createResourceEndpoint,
		GetResourceEndpoint:    getResourceEndpoint,
		ListResourcesEndpoint:  listResourcesEndpoint,
		UpdateResourceEndpoint: updateResourceEndpoint,
		DeleteResourceEndpoint: deleteResourceEndpoint,
	}
...
func RegisterService(cfg nserver.Config, r *mux.Router, s *grpc.Server) endpoint.Endpoints {
	...
	pb.RegisterCargoServer(s, grpc2.MakeGRPCServer(endpoints, tracer, logger))
...
func Run(cfg nserver.Config) {
	...
	// Run!
	logs.Info("cargo.v1.CargoServer", " started.")
	logs.Info("cargo.v1.CargoServer", <-errc)
	logs.Info("cargo.v1.CargoServer", " closed.")
}
