...
import (
	...
	// This Service
	"github.com/chaos-io/cargo/backend/modules/cargo-server/api"
)
...
func main() {
	...
	err := config.ScanFrom(&conf, "cargoServer", "server")
	if err != nil {
	...
	if len(gitHash) == 0 {
		logs.Infow("starting CargoServer, you can use `-ldflags \"-X main.gitHash=xxx\" to generate gitHash")
	} else {
